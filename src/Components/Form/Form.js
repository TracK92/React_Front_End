...
// fetch data from backend
  ...
  const createAccessCode = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:4000/createNewAccessCode", post);
      console.log(res);
    } catch (err) {
      console.log(err);
    }
  ...
  const validateAccessCode = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:4000/validateAccessCode", post);
      console.log(res);
      ...
      } else if (res.data["message"] === "Access code and phone number are required") {
  ...
  return (
    <form>
      ...
      />
      {/* submit button */}
      <button type="button" onClick={validateAccessCode}>Validate Access Code</button>
    </form>
  );
};
